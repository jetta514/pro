...
style_add() {
	...
	mkdir -p ~/.termux
	wget https://github.com/romkatv/powerlevel10k-media/raw/master/MesloLGS%20NF%20Regular.ttf -O ~/.termux/font.ttf
...
show_menu() {
	while true; do
		...
		echo -e "     ${YELLOW}[3]${RESET} ${GREENB}${BLACK}Установка Fish и Oh-My-Fish${RESET}"
		echo -e "     ${YELLOW}[4]${RESET} ${GREENB}${BLACK}Установка шрифтов и цветовых схем${RESET}"
		echo -e "     ${YELLOW}[5]${RESET} ${GREENB}${BLACK}Установка Daijin${RESET}"
		echo -e "     ${YELLOW}[6]${RESET} ${GREENB}${BLACK}Установка Udocker${RESET}"
		echo -e "     ${YELLOW}[7]${RESET} ${YELLOWB}${PURPLE}Установка eza, bat, fastfetch${RESET}"
